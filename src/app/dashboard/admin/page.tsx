...
function AdminDashboardContent() {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'companies' | 'users' | 'teams' | 'blogs' | 'settings' | 'agents' | 'demat' | 'customer_kyc' | 'feedback' | 'analytics' | 'orders' | 'notifications' | 'leads' | 'rm_targets' | 'audit_log' | 'price_history' | 'agent_performance' | 'market_intel' | 'mf_interests' | 'profile'>('overview');

    useEffect(() => {
        const tab = searchParams.get('tab');
        const validTabs = ['overview', 'companies', 'users', 'teams', 'blogs', 'settings', 'agents', 'demat', 'customer_kyc', 'feedback', 'analytics', 'orders', 'notifications', 'leads', 'rm_targets', 'audit_log', 'price_history', 'agent_performance', 'market_intel', 'mf_interests', 'profile'];
    ...
    return (
        <div className="container mx-auto px-4 md:px-8 py-8 relative">
            ...
            {activeTab === 'profile' && <ProfileTab roleLabel="Administrator" />}
